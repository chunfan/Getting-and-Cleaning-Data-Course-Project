...
meanS <- as.data.frame(t(sapply(s, function(x) colMeans(x[,2:(ncol(x)-1)]))))
...
tidyData <- data.frame(subject=factor(rep(levels(data$subject), each=nlevels(data$activity))), 
                  activity=factor(rep(levels(data$activity),times=nlevels(data$subject))))
tidyData <- cbind(tidyData, meanS)
